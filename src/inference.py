...
def evaluate_roc(net: nn.Module, test_loader: DataLoader, device: str = 'cuda', num_classes: int = 4):
    """
...
def main():
    ...
    num_classes = 4  # Обновлено для обработки 4 классов
    fpr, tpr, roc_auc, all_labels, all_preds = evaluate_roc(net, test_dataloader, device=device, num_classes=num_classes)

    # Plot ROC Curves
    plt.figure()
    colors = ['aqua', 'darkorange', 'cornflowerblue', 'green']  # Добавлен цвет для 4-го класса
    for i in range(num_classes):
